...
const AddProfessor = () => {
    ...
    const addProfessor = (name: string, school: string, faculty: string) => {
        try{
            console.log(name, school, faculty)
            if(!faculty || !name || !school){
                throw "Incomplete Data"
            }
            onClose()
            axios
            .post("../api/form/add-professor", {
                name: name,
                school: school,
                faculty: faculty,
            })
                    ...
                    <div className="flex flex-col w-full tablet:mx-4 h-full">
                        ...
                        <div className={`rounded-full relative flex flex-col bg-white border-2 border-mariana px-4 py-2 gap-2 `}>
                            <div className="flex flex-row items-center gap-2 h-full ">
                                <input required onInput={e => setName((e.target as HTMLInputElement).value)} value={name} type="setName" className="outline-none text-xs tablet:text-lg placeholder:text-xs placeholder:tablet:text-lg placeholder:text-mariana font-hauora" placeholder={`John Doe`} />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <ModalBody>
                        <div className="flex flex-col items-center justify-center my-8 gap-6">
                            ...
                            <div className="flex flex-row gap-4">
                                <Button preset="primary" onClick={() => addProfessor(name, school, faculty)}>Add Lecturer</Button>
